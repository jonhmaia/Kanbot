    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex items-center justify-between gap-2 px-3 pb-2 pt-1">
        <div className="min-w-0">
          ...
          <p className="truncate text-[13px] text-chalk/90">{watching ? 'Vendo a tela' : 'Chat'}</p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-full text-dust hover:bg-white/[0.06] hover:text-chalk"
            aria-label="Recolher"
          >
            <IconClose size={14} />
          </button>
        ...
        {messages.map((message, index) => (
          <div
            ...
            className={
              ...
              (message.role === 'user' ? 'ml-auto max-w-[88%] bg-white/[0.09] text-chalk' : 'border border-white/[0.06] bg-white/[0.04] text-dust')
            }
          >
            {message.role === 'bot' && (
            ...
            {message.text}
            {message.sawScreen && (
              <p className="mt-1 text-[10px] text-smoke">Com print da tela</p>
            )}
          </div>
        ))}
        {thinking && (
      ...
      <div className="scroll-slim flex gap-1.5 overflow-x-auto px-3 pt-2">
        {chips.slice(0, 4).map((chip) => (
          <button
            ...
            className="shrink-0 rounded-full border border-white/10 px-2.5 py-1 text-[11px] text-dust hover:text-chalk"
          >
            {chip}
          </button>
        ))}
      ...
      >
        <input
          ...
          className="min-w-0 flex-1 rounded-full bg-white/[0.06] px-3 py-2 text-[13px] text-chalk outline-none placeholder:text-smoke"
        />
        <button
          type="submit"
          disabled={thinking || (!value.trim() && !(watching && frame))}
          className="grid h-9 w-9 place-items-center rounded-full text-[#111] disabled:opacity-40"
