...
create policy "status master legivel" on public.master_statuses
  for select using (public.is_workspace_member(workspace_id));
create policy "status master editavel" on public.master_statuses
  for all using (public.can_edit_workspace(workspace_id))
  with check (public.can_edit_workspace(workspace_id));

create policy "projetos legiveis" on public.projects
  for select using (public.is_workspace_member(workspace_id));
create policy "projetos editaveis" on public.projects
  for all using (public.can_edit_workspace(workspace_id))
  with check (public.can_edit_workspace(workspace_id));
...
create policy "colunas editaveis" on public.board_columns
  for all using (public.can_edit_workspace(public.workspace_of_project(project_id)))
...
create policy "tarefas legiveis" on public.tasks
  for select using (public.is_workspace_member(public.workspace_of_project(project_id)));
create policy "tarefas editaveis" on public.tasks
  for all using (public.can_edit_workspace(public.workspace_of_project(project_id)))
  with check (public.can_edit_workspace(public.workspace_of_project(project_id)));
...
create policy "checklist legivel" on public.checklist_items
  for select using (public.is_workspace_member(public.workspace_of_task(task_id)));
create policy "checklist editavel" on public.checklist_items
  for all using (public.can_edit_workspace(public.workspace_of_task(task_id)))
  with check (public.can_edit_workspace(public.workspace_of_task(task_id)));

create policy "anexos legiveis" on public.attachments
  for select using (public.is_workspace_member(public.workspace_of_task(task_id)));
create policy "anexos editaveis" on public.attachments
  for all using (public.can_edit_workspace(public.workspace_of_task(task_id)))
  with check (public.can_edit_workspace(public.workspace_of_task(task_id)));

create policy "labels legiveis" on public.labels
  for select using (public.is_workspace_member(workspace_id));
create policy "labels editaveis" on public.labels
  for all using (public.can_edit_workspace(workspace_id))
  with check (public.can_edit_workspace(workspace_id));

create policy "task_labels legiveis" on public.task_labels
  for select using (public.is_workspace_member(public.workspace_of_task(task_id)));
create policy "task_labels editaveis" on public.task_labels
  for all using (public.can_edit_workspace(public.workspace_of_task(task_id)))
  with check (public.can_edit_workspace(public.workspace_of_task(task_id)));
...
create policy "insights legiveis" on public.ai_insights
  for select using (public.is_workspace_member(workspace_id));
create policy "insights editaveis" on public.ai_insights
  for all using (public.can_edit_workspace(workspace_id))
  with check (public.can_edit_workspace(workspace_id));
...
create or replace view public.v_master_board as
select ms.workspace_id, ms.key as status_key, ms.name as status_name,
       ms.color, ms.position, t.*
...
create or replace view public.v_member_workload as
